-- SECRET VILLAGE SECRET CHAIN v2
-- Turns early discoveries into a connected mystery progression.
-- Idempotent world setup; clues never grant currency or duplicate secret rewards.
...
local root=Workspace:FindFirstChild("SECRET_CHAIN") or Instance.new("Folder")
root.Name="SECRET_CHAIN"
...
local function marker(name,pos,text,need,clue)
 ...
 p.Transparency=.18
 p:SetAttribute("SecretChainMarker",true)
 p.Parent=root

 local b=Instance.new("BillboardGui")
 b.Size=UDim2.fromOffset(260,55)
 ...
 pr.Triggered:Connect(function(player)
  ...
  local count=math.clamp(math.floor(tonumber(player:GetAttribute("SecretsFound")) or 0),0,100)
  if count<need then
   Notify:FireClient(player,"🔒 Нужно найти ещё секретов: "..need)
   return
  end
...
local function connectPlayer(p)
 p:GetAttributeChangedSignal("SecretsFound"):Connect(function()
  local n=math.floor(tonumber(p:GetAttribute("SecretsFound")) or 0)
  if n==1 or n==2 or n==4 or n==5 or n==7 or n==9 then
   Notify:FireClient(p,"📖 Книга секретов обновилась. Проверь новые подсказки!")
  end
 end)
end
