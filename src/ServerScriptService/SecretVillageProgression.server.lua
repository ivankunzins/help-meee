...
local function load(player)
	...
	if type(data) ~= "table" then
		...
		if oldOk and type(oldData) == "table" then
			data = oldData
		else
			data = {}
		end
	...
	player:GetAttributeChangedSignal("QuestXP"):Connect(function()
		...
		if newLevel > oldLevel then
			Notify:FireClient(player, "⭐ НОВЫЙ УРОВЕНЬ! Уровень " .. newLevel .. ".")
		end
	end)
end

local function save(player)
	if not player or not player.Parent or not loaded[player] or saving[player] then
		return false
	end
	...
	if not success then
		warn("[SecretVillageProgression] Save failed for " .. player.Name)
	end
...
event.Event:Connect(function(player)
	if player and player:IsA("Player") and loaded[player] then
