-- SECRET VILLAGE OWNERSHIP v3
-- Persistent ownership with validation, retries, save locking and stale-key cleanup.
...
local loaded={}
local saving={}
local allowed={VillageCar=true,Taxi=true,DeliveryVan=true,Flashlight=true,Detector=true,Diving=true,MagicCarpet=true,MysteryBox=true}

local function isValidKey(key)
 if type(key)~="string" or #key<=4 or #key>80 or string.sub(key,1,4)~="Own_" then return false end
 return allowed[string.sub(key,5)]==true
end
...
local function load(player)
 ...
 if type(data)=="table" then
  for key,value in pairs(data) do
   if isValidKey(key) and value==true then player:SetAttribute(key,true) end
  end
 end
 loaded[player]=true
...
local function collectOwnership(player)
 ...
 for key,value in pairs(player:GetAttributes()) do
  if isValidKey(key) then data[key]=value==true end
 end
 return data
end
...
local function save(player)
 ...
 for attempt=1,3 do
  local ok=pcall(function()
   Store:UpdateAsync("u_"..player.UserId,function(old)
    ...
    for key,value in pairs(data) do
     if isValidKey(key) then old[key]=value end
    end
    for key in pairs(old) do
     if isValidKey(key) and data[key]==nil then old[key]=nil end
    end
    return old
   end)
  end)
 ...
 if not success then warn("[SecretVillageOwnership] Save failed for "..player.Name) end
 saving[player]=nil
 return success
end
