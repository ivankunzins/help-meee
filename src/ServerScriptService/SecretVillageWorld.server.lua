...
build()
task.spawn(function()while true do task.wait(math.random(240,420));local e=math.random(1,5);if e==1 then Workspace:SetAttribute("NightEvent",true);Lighting.ClockTime=0;notifyAll("🌙 НОЧЬ: открылись тайны, которых нет днём!");task.wait(90);Workspace:SetAttribute("NightEvent",false);Lighting.ClockTime=14 elseif e==2 then notifyAll("🌫️ ТУМАН: деревня стала огромной загадкой!");Lighting.FogEnd=120;task.wait(75);Lighting.FogEnd=100000 elseif e==3 then Workspace:SetAttribute("BlackoutEvent",true);notifyAll("⚡ ОТКЛЮЧЕНИЕ: ищи тайный свет!");task.wait(60);Workspace:SetAttribute("BlackoutEvent",false)elseif e==4 then Workspace:SetAttribute("TraderEvent",true);notifyAll("🚚 ТОРГОВЕЦ: появился редкий товар!");task.wait(120);Workspace:SetAttribute("TraderEvent",false)else notifyAll("🔔 ДЕРЕВЕНСКОЕ СОБЫТИЕ: кто-то видел странный объект у реки...")end end end)
