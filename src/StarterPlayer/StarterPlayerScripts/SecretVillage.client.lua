-- SECRET VILLAGE — unified mobile/PC HUD v3
local Players=game:GetService("Players")
local ReplicatedStorage=game:GetService("ReplicatedStorage")
local player=Players.LocalPlayer
local remotes=ReplicatedStorage:WaitForChild("SecretVillageRemotes")
local Notify=remotes:WaitForChild("Notify");local Hint=remotes:WaitForChild("BuyHint");local StartJob=remotes:WaitForChild("StartJob");local EndJob=remotes:WaitForChild("EndJob");local SpawnVehicle=remotes:WaitForChild("SpawnVehicle");local GetSecretStatus=remotes:WaitForChild("GetSecretStatus")
...
local buttons=panel(UDim2.fromOffset(235,254),UDim2.new(0,14,1,-272))
...
button(93,"🚗 Машина",function()SpawnVehicle:FireServer("VillageCar")end)
button(137,"🚕 Такси — $500",function()SpawnVehicle:FireServer("Taxi")end)
button(181,"📦 Фургон — $750",function()SpawnVehicle:FireServer("DeliveryVan")end)
local book
local bookButton=button(225,"📖 Книга тайн",function()book.Visible=not book.Visible;if book.Visible then refreshBook()end end)
...
local function update()
 ...
 local n=player:GetAttribute("SecretsFound")or 0;secrets.Text="🔐 "..n.." / 100";jobButton.Text=player:GetAttribute("InJob")and "🛑 Закончить смену"or "🧹 Работа — $100";objectiveText.Text=n>=50 and "👑 Цель: собрать половину коллекции" or n>=10 and "🏆 Цель: MASTER EXPLORER" or n>0 and "🎯 Цель: найди следующий секрет" or "🎯 Цель: найди первый секрет"
end
